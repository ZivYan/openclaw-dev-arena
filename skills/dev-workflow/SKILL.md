...
**每一步都必须在群聊发送思考过程和结论，禁止长时间沉默工作。**

每个 Phase 必须发送：
1. **进入通知** — 进入新 Phase 时立即告知用户
2. **思考过程** — 正在做什么、读了哪些代码、发现了什么、判断依据是什么
3. **结论输出** — 该 Phase 的产出和决策
...
**执行步骤**：
1. **群聊通知**：`🔄 进入 Phase 0：需求理解，正在分析需求...`
2. 分析用户描述，提炼核心诉求
3. 阅读相关代码，**群聊同步发现**：读了哪些文件、现有实现是什么样的、影响面初步判断
4. 输出结构化需求摘要：
...
1. **群聊通知**：`🔄 进入 Phase 1：技术方案设计（对抗式），正在整理需求 brief...`
2. 将 Phase 0 确认的需求整理为结构化 brief，包含：目标、约束、影响面、验收标准、相关代码路径。
...
2. 群聊通知：「📤 已向 arch-alpha（守正）发送需求 brief，等待方案 A...」
3. sessions_send → arch-alpha：发送需求 brief，要求独立出方案 A
4. 收到方案 A → 群聊通知：「📥 收到 arch-alpha 方案 A：[一句话摘要]，正在转发给 arch-beta...」
5. sessions_send → arch-beta：发送需求 brief + 方案 A，要求出方案 B + rebuttal A
6. 收到方案 B → 群聊通知：「📥 收到 arch-beta 方案 B：[一句话摘要] + rebuttal，正在转发给 arch-alpha 回应...」
7. sessions_send → arch-alpha：转发 rebuttal，要求回应
8. 收到回应 → 群聊通知：「📥 收到 arch-alpha 回应，正在判断是否对齐...」
```

### 1.3 对抗收敛

检查双方是否已对齐（核心分歧是否消除）：
- **已对齐** → 群聊通知：`✅ 双方已对齐，正在汇总裁决...` → 进入 1.4
- **未对齐** → 群聊通知：`🔄 核心分歧仍存在：[分歧点]，继续第 N 轮对抗...` → 将回应转发给 arch-beta，继续（最多 3 轮额外对抗）
...
1. **群聊通知**：`🔄 进入 Phase 2：分支创建 + 任务拆分`

检查用户是否指定了分支：
- **已指定** → 群聊通知：`📌 切换到指定分支 <branch>` → 切换
- **未指定** → 群聊通知：`🌿 未指定分支，从 master 拉新分支 feat/xxx-MMDD` → 从 master 拉新分支
...
**进入时群聊通知**：`🔄 进入 Phase 3：原子开发，共 N 个任务`

**每个任务的执行流程**：
1. **群聊通知开始**：`🔨 开始 [1/5] 实现 GetProfile 方法，涉及文件：service/user.go, dal/user.go`
2. 定义/修改接口
3. 编写实现代码
4. **群聊同步思考**：遇到关键决策时说明选择和理由（如：「选择用 sync.Map 而非加锁，因为读多写少场景」）
5. 编写对应单测
6. 运行单测确认通过
7. git commit（一个任务一个 commit）
...
**群聊通知**：`🔄 进入 Phase 4：自测验证，正在识别技术栈并执行全量测试...`
...
**群聊通知**：`🔄 进入 Phase 5：Code Review，正在按 P0-P3 优先级审查代码...`
...
**群聊通知**：`🔄 进入 Phase 6：提交 MR，正在 rebase master 并准备提交...`
...
**群聊通知**：`🔄 进入 Phase 7：提测交付，正在生成提测报告并归档到飞书文档...`
